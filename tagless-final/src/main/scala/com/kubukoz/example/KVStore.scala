...
object KVStore {
  ...
  def redisImpl[F[_], K, V](implicit commands: StringCommands[F, K, V], del: KeyCommands[F, K]): KVStore[F, K, V] = new KVStore[F, K, V] {
...
final class KVStoreLaws[F[_]: Monad, K, V](store: KVStore[F, K, V]) {
  ...
  def writeGetPersistence(k: K, v: V) =
    write(k, v) *> get(k) <-> write(k, v) *> v.some.pure[F]

  def latestOverwriteWins(k: K, vOld: V, v: V) =
    write(k, vOld) *> write(k, v) *> get(k) <-> write(k, v) *> get(k)

  def deleteRemoves(k: K, v: V) =
    write(k, v) *> delete(k) *> get(k) <-> none[V].pure[F]
